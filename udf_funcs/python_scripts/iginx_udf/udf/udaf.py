...
from .udf_base import UDFBase, get_column_index, get_constants
from ..utils.dataframe import DataFrame, pandasDF_to_list, list_to_PandasDF


class UDAF(UDFBase, ABC):
    """
    参数列举模式
    """
    def __init__(self):
    ...
    def call_func(self, data, pos_args, kwargs):
        ...
        for row in data[2:]:
            ...
            self.status = self.transform(self.status, *args, **kwargs)
